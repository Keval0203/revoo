...
export function validateBookingCreationInput(requestBody: any): {
  ...
  const { facilityId, courtId, timeSlotId, selectedTimeSlotIds, userDetails } = requestBody;

  // Extract selected slot IDs
  const slotIds: string[] = Array.isArray(selectedTimeSlotIds) && selectedTimeSlotIds.length > 0
    ? selectedTimeSlotIds
  ...
  if (slotIds.length === 0) errors.push("At least one time slot is required");
  ...
  return {
    ...
    sanitizedInput: {
      ...requestBody,
      selectedTimeSlotIds: slotIds,
      timeSlotId: slotIds[0] || timeSlotId,
      userDetails: {
        ...userDetails,
        phone: userDetails.phone ? userDetails.phone.replace(/\s+/g, '') : '',
      },
    },
  };
}
