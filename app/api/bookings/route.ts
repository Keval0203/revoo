  ...
  try {
    if (!process.env.DATABASE_URL?.trim()) {
      ...
      const bookings = status && status !== 'all'
        ? demoBookings.filter((booking) => booking.status === status)
        : demoBookings;
      return Response.json({
        bookings,
        pagination: { page: 1, limit: bookings.length, total: bookings.length, totalPages: 1, hasMore: false },
        statusCounts: {
          ...
          COMPLETED: demoBookings.filter((booking) => booking.status === 'COMPLETED').length,
          CANCELLED: 0,
        },
      });
    }
