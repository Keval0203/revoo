...
export async function GET(
  ...
  if (!process.env.DATABASE_URL?.trim()) {
    ...
    return Response.json({
      booking: {
        ...
        user: { id: 'demo-user', name: 'Demo Player', email: 'player@revo.demo', phone: '+91 90000 00000' },
        facility: { ...booking.facility, description: 'A demo sports facility', state: 'Maharashtra', pincode: '400001', images: [] },
        court: { ...booking.court, description: 'Demo court', hourlyRate: booking.court.pricePerHour },
      },
    });
  }
  ...
  try {
    ...
    const transformedBooking = {
      ...
      timeSlots: booking.timeSlots && booking.timeSlots.length > 0 ? booking.timeSlots.map(ts => ({
        id: ts.id,
        startTime: ts.startTime ? ts.startTime.toISOString().split('T')[1]?.split('.')[0] : null,
        endTime: ts.endTime ? ts.endTime.toISOString().split('T')[1]?.split('.')[0] : null,
        price: ts.price,
        date: ts.date ? ts.date.toISOString().split('T')[0] : null,
      })) : [],
      ...
      facility: {
        ...
        country: booking.facility.country,
        pincode: booking.facility.pincode,
...
export async function PATCH(
  ...
  if (!process.env.DATABASE_URL?.trim()) {
    ...
    if (body.action === 'cancel' && booking.status === 'CONFIRMED') {
      return Response.json({ message: 'Booking cancelled successfully', booking: { id: bookingId, status: 'CANCELLED', cancelledAt: new Date().toISOString() } });
    }
    return Response.json({ error: 'This demo booking action is unavailable' }, { status: 400 });
  }
  ...
  try {
    ...
    if (action === 'cancel') {
      ...
      // Update all associated time slots to be available again
